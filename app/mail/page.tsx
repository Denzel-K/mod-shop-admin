...
export default function MailPage() {
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      ...
      const idx = selected ? formatted.findIndex(m => m._id === selected._id) : -1;
      if (e.key === 'j') {
        const next = formatted[Math.min(formatted.length - 1, idx + 1)] || formatted[0];
        if (next) setSelected(next);
      } else if (e.key === 'k') {
        const prev = formatted[Math.max(0, idx - 1)] || formatted[formatted.length - 1];
      ...
      <main className="px-4 sm:px-6 py-6">
        ...
        <Dialog open={open} onOpenChange={setOpen}>
          <DialogContent className="sm:max-w-2xl lg:max-w-4xl bg-slate-900 border border-slate-800 text-white p-0 overflow-hidden">
            ...
            {/* Thread */}
            <div ref={scrollRef} className="max-h-[70vh] overflow-y-auto p-4 sm:p-6 space-y-3">
              {bubbles.map((b) => (
